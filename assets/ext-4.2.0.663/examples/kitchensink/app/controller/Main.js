Ext.define('KitchenSink.controller.Main', {
    ...
    init: function() {
	    //var cls=this.getContentPanel().xtype;
        this.control({
            'navigation': {
			...
			'contentPanel button[action=dummy]': {
				click:function(){
				  
				  alert(this.getCustomerpanel().down('grid[Cls=aaa]').getSelectionModel().getSelection()[0].get('fda'));
				}
			},
			'customer-panels button[action=btna]':{
				click:this.btna
			}
        });
    },
